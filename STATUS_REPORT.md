...
| Arrays | One-dimensional arrays, default initialization, explicit/implicit initializers, length, indexing/mutation, and iteration. Initializer inserts are batched and split at SQL Server's 1,000-row `VALUES` limit. | Multidimensional arrays produce `SS6301` (`src/SharpSql/SharpSqlCompiler.Heap.Creation.cs:674-686`). |
