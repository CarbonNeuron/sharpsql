...
`byte[]` is the deliberate exception to the indexed-array representation: it remains a native `VARBINARY(MAX)` scalar through IR and SQL lowering. Construction, `Length`, indexed reads and writes, `foreach`, and `SequenceEqual` therefore avoid per-element heap rows. This is compact and keeps binary database interop direct, but assignment aliases and dictionary keys currently follow binary content/value semantics rather than CLR array reference identity.
